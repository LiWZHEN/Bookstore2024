...
namespace keyword {
  ...
  void insert(const std::string &Keyword, const std::string &ISBN) {
    bool exist = false;
    std::fstream file("keyword.txt");
    if (file.is_open()) {
      exist = true;
    }
    file.close();
    if (!exist) {
      std::ofstream new_file("keyword.txt");
      new_file.close();
    }
    ...
    const std::string &index_str = Keyword;
  ...
  void erase(const std::string &Keyword, const std::string &ISBN) {
    bool exist = false;
    std::fstream file("keyword.txt");
    if (file.is_open()) {
      exist = true;
  ...
  std::vector<std::string> find(const std::string &Keyword) {
