...
namespace author {
  ...
  void insert(const std::string &Author, const std::string &ISBN) {
    bool exist = false;
    std::fstream file("author.txt");
    if (file.is_open()) {
      exist = true;
    }
    file.close();
    if (!exist) {
      std::ofstream new_file("author.txt");
      new_file.close();
    }
    ...
    const std::string &index_str = Author;
  ...
  void erase(const std::string &Author, const std::string &ISBN) {
    bool exist = false;
    std::fstream file("author.txt");
    if (file.is_open()) {
      exist = true;
  ...
  std::vector<std::string> find(const std::string &Author) {
